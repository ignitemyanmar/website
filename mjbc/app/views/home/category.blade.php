					...
					<div class="row list_panel">
						<div class="large-4 medium-4 columns">
								@if($row->image) 
									...
									@else
										<img src="../../seminar_img/php/files/medium/{{$row->image}}"> 
									@endif
								@else 
									<img src="../../img/img.png"> 
								@endif
						...
						<div class="large-8 medium-8 columns nopadding">
							...
							<div class="clear">&nbsp;</div>
							@if($language=="japan")
								<a href="/type/en/{{str_replace('/','-',$type)}}/{{$row->id}}/{{$row->name}}"><span class="more">続き</span></a>
							@elseif($language=="myanmar")
								<a href="/type/en/{{str_replace('/','-',$type)}}/{{$row->id}}/{{$row->name}}"><span class="more">အေသးစိတ္ၾကည့္ရန္</span></a>
							@else

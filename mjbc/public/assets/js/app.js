var App = function () {
    ...
    var handleLoginForm = function () {
        ...
        $('.register-form').validate({
            ...
            submitHandler: function (form) {
                ...
                var rpassword=$('#register_password').val();
                $('.loader').show();
                $.ajax({
                  type: "post",
                  url: "/user-register",
                  data: {username:rusername, email:remail, password:rpassword}
                    }).done(function( result ) {
                      if(result=='Email is already used!' || result =="Sorry. You can't register."){
                        alert(result);
                        $('.loader').hide();
                        return false;
                      }else{
    ...
    var handleClockfaceTimePickers = function () {
        ...
        $('#clockface_endtime').clockface({ 
            format: 'HH:mm',
            trigger: 'manual'
        });///new for endtime
        
        $('#endtime-btn').click(function (e) { 
            e.stopPropagation();
            $('#clockface_endtime').clockface('toggle');
        });///new for endtime
    ...
    var handleFormWizards = function () {
        ...
        $('#form_wizard_1').bootstrapWizard({
            ...
            onNext: function (tab, navigation, index) {
                ...
                var sizes= $('.size').length;
                var htmlresult='';
                jQuery('.images').html('');
                var obj_colors=[];
                for(var i=0; i < colors; i++){
                    if(i==0){
                        obj_colors.push({color:objcolors[i].value});
                    }else{
                        var exit=0;
                        for(var j=0; j<obj_colors.length; j++){
                            if(obj_colors[j].color == objcolors[i].value){
                                exit +=1;
                            }else{}
                        } 
                        if(exit ==0 && objcolors[i].value !=null){
                            obj_colors.push({color:objcolors[i].value});
                        }  
                    }
                }
                var string=JSON.stringify(obj_colors);
                var totalimagecount =obj_colors.length;
                var imagediv='';
                for(var j=0; j<totalimagecount; j++){
                    var classnamefordiv='gallery_container'+obj_colors[j].color;
                    var uploadthumbdiv='upload'+obj_colors[j].color;
                    var color=obj_colors[j].color;
                    var preview="preview"+color;
                    var image_file="image_file"+color;
                    var error="error"+color;
                    var warnsize="warnsize"+color;
                    var hidden_img="hidden_img_"+color;
                    var onchange="fileSelect('"+preview+"','"+image_file+"','"+error+"','"+warnsize+"','"+hidden_img+"')";
                    imagediv +='<label class="control-label">Image for '+obj_colors[j].color+'</label>';
                    imagediv +='<div class="control-group">'+
                                // '       <label class="control-label"></label>'+
                                '       <div class="controls">'+
                                '             <div class="span2 file-upload">'+
                ...
                jQuery('.images').append(imagediv);

                // alert(totalimagecount);

...
class MobileController extends BaseController
{
	...
	public function getconditionlist(){
		$condition=ConMobileControllerdition::all();
		...
		if($car){
			foreach ($car as $row) {
				...
				switch ($row->transmission) {
					case 'Automatic':
						$temp['transmissionid']=1;
						break;
					case 'Manual':
						$temp['transmissionid']=2;
						break;
					case 'Auto-manual':
						$temp['transmissionid']=3;
						break;
					default:
						$temp['transmissionid']=4;
						break;
				}
				...
				// $temp['enginepower']=EnginePower::whereid($row->enginepower_id)->pluck('name');
        ...
        if($linkparas) 
        {
			...
			if($cars){
				foreach ($cars as $row) {
					...
					$main_image='';
					if($row->image){
						$thumbnailimage='carphoto/php/files/thumbnail/'.$row->image;
						$main_imageurl='carphoto/php/files/'.$row->image;
						if(File::exists($main_imageurl)){
							if(File::exists($thumbnailimage)){
							}else{
								$img=Image::make('carphoto/php/files/'.$row->image);
								$img->resize(200, null, function ($constraint) {
								    $constraint->aspectRatio();
								})->save($thumbnailimage);
							}
						}
						$main_image=str_replace(' ', '%20', $row->image);

						$main_image='carphoto/php/files/thumbnail/'.$main_image;
					}
					// File::exit()
    ...
    */
    ...
    public function getsearch(){
        ...
        public function postuserregister(){
        	...
        	if(count($check)>0){
        		$userinfo['status']=0;
        		$userinfo['message']='Email is already in use.';
        		return Response::json($userinfo);
        	}
        	$objuser=new User();
	...
	public function getmodellistbybrand($makeid){
		...
		if(count($modelids)>0){
			$models=Models::wherein('id', $modelids)->orderBy('model','asc')->get();
		}
		return Response::json($models);
	}
}